...
set(_lh_installed_marker "${_lh_install_prefix}/bin/.lh_install_marker")
if(NOT EXISTS "${_lh_installed_marker}" OR IS_DIRECTORY "${_lh_installed_marker}")
    message(FATAL_ERROR
        "安装布局缺少安装标记文件: ${_lh_installed_marker}"
    )
endif()
...
if(WIN32 AND LH_ENABLE_QT_DEPLOYMENT)
    foreach(_lh_plugin_stem IN ITEMS "platforms/qwindows" "sqldrivers/qsqlite")
        ...
        if(NOT EXISTS "${_lh_plugin_expected}" AND NOT EXISTS "${_lh_plugin_fallback}")
            message(FATAL_ERROR
                "Windows Qt 部署配置 '${_lh_install_config_label}'（模式 ${_lh_qt_deploy_mode}）"
                "缺少期望插件文件: ${_lh_plugin_expected}"
            )
        endif()
    endforeach()
elseif(WIN32)
