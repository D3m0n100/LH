...
private slots:
    ...
    void saveProjectPersistsDslWhenAuxiliaryWindowIsActive()
    {
        ...
        auxiliaryWindow->setProperty("modified", true);
    ...
    void offlineCompileClearsOldDownloadArtifactWithoutPublishing()
    {
        QTemporaryDir temp;
        ...
        QVERIFY(project);
        QVERIFY(build);
        QVERIFY(project->openProjectFromPath(path));
        project->runtimeConfig().downloadArtifact.filePath = QStringLiteral("old.code");
        QSignalSpy success(build, &BuildController::compileSucceeded);
        QSignalSpy failure(build, &BuildController::compileFailed);
        build->compileConfiguration(path, project->runtimeConfig());
        QTRY_VERIFY_WITH_TIMEOUT(success.count() + failure.count() > 0, 30000);
        ...
        QVERIFY(project->runtimeConfig().downloadArtifact.filePath.isEmpty());
        QVERIFY(!RunController::writeDownloadArtifact(project->runtimeConfig(), path, result));
        QFile saved(QDir(path).filePath(QStringLiteral("project_config.json")));
        QVERIFY(saved.open(QIODevice::ReadOnly));
        QVERIFY(QJsonDocument::fromJson(saved.readAll()).object()
                .value(QStringLiteral("downloadArtifact")).toObject()
                .value(QStringLiteral("filePath")).toString().isEmpty());
    }

    void repeatedSnippetSignalPathsAreNotExclusiveBindings()
    {
        ProjectController controller;
        auto& config = controller.runtimeConfig();
        config.clear();
        config.projectName = QStringLiteral("repeated-snippets");
        for (int i = 0; i < 3; ++i) {
            DslMappingEntry mapping;
            mapping.id = QStringLiteral("mapping-%1").arg(i);
            mapping.snippetId = QStringLiteral("_DealToData");
            mapping.signalPath = QStringLiteral("data._DealToData");
            mapping.periodMs = 20;
            config.dslMappings.append(mapping);
        }
        QStringList errors;
        QVERIFY2(controller.validateConfiguration(errors), qPrintable(errors.join("\n")));
        config.dslMappings[1].id = config.dslMappings[0].id;
        QVERIFY(!controller.validateConfiguration(errors));
        QVERIFY(errors.join("\n").contains(QStringLiteral("mapping-0")));
        config.dslMappings[1].id = QStringLiteral("mapping-1");
        HardwareResourceBinding resource;
        resource.id = QStringLiteral("resource-1");
        resource.resourceType = QStringLiteral("AI");
        resource.channel = QStringLiteral("0");
        config.resources.append(resource);
        resource.id = QStringLiteral("resource-2");
        config.resources.append(resource);
        QVERIFY(!controller.validateConfiguration(errors));
        QVERIFY(errors.join("\n").contains(QStringLiteral("AI:0")));
    }
