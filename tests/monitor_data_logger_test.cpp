...
class CrossThreadLoggerCaller final : public QThread
{
public:
    explicit CrossThreadLoggerCaller(MonitorDataLogger* logger)
        : m_logger(logger)
    {
    }
...
protected:
    ...
    {
        m_logger->flush();
        flushError = m_logger->lastError();
        m_logger->shutdown();
        shutdownError = m_logger->lastError();
    }

private:
    MonitorDataLogger* m_logger = nullptr;
};

class CrossThreadDisableCaller final : public QThread
{
public:
    explicit CrossThreadDisableCaller(MonitorDataLogger* logger)
        : m_logger(logger)
    {
    }
...
class CrossThreadEnqueueCaller final : public QThread
{
public:
    CrossThreadEnqueueCaller(MonitorDataLogger* logger, const Sample& sample)
        : m_logger(logger)
        , m_sample(sample)
    {
    }
...
protected:
    void run() override
    {
        m_logger->enqueueSample(m_sample);
    }
...
private slots:
    ...
    void disablingFlushesBufferedInvalidSampleWithQuality()
    {
...

        {
            ...
            QVERIFY(logger.m_buffer.isEmpty());
            QVERIFY(!logger.m_flushTimer->isActive());

            CrossThreadEnqueueCaller caller(
                &logger,
                makeSample(QStringLiteral("disabled.admission.point"),
                           RuntimePointQuality::Good,
                           true,
                           8.0));
            caller.start();
            QVERIFY(caller.wait(500));
            QVERIFY(logger.m_buffer.isEmpty());
    ...
    void oversizedBatchKeepsLimitAndReportsDroppedCount()
    {
        ...
        QCOMPARE(logger.droppedSampleCount(), quint64(3));
    }
    ...
    void failedFlushBacksOffAndSuccessfulRetryResetsDelay()
    {
        auto& dataManager = DataManager::instance();
        const QString databasePath = m_tempDir.path() + QStringLiteral("/backoff_reset.db");

        MonitorDataLogger logger;
        logger.setBatchSize(1000);
        logger.enqueueSample(makeSample(QStringLiteral("backoff.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         3.0));

        logger.flush();
        QCOMPARE(logger.retryDelayMs(), MonitorDataLogger::DEFAULT_FLUSH_INTERVAL_MS);
        QCOMPARE(logger.lastError(), QStringLiteral("DataManager not initialized"));

        for (int i = 0; i < 10; ++i) {
            logger.flush();
        }
        QCOMPARE(logger.retryDelayMs(), MonitorDataLogger::MAX_RETRY_DELAY_MS);

        QVERIFY(dataManager.initialize(databasePath));
        logger.flush();
        QCOMPARE(logger.retryDelayMs(), logger.flushIntervalMs());
        QVERIFY(logger.lastError().isEmpty());
        QCOMPARE(allRecords(QStringLiteral("backoff.point")).size(), 1);
    }
    ...
    void crossThreadOperationsReturnImmediately()
    {
        MonitorDataLogger logger;
        logger.enqueueSample(makeSample(QStringLiteral("cross_thread.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         5.0));

        CrossThreadLoggerCaller caller(&logger);
        caller.start();
        QVERIFY(caller.wait(500));
        QVERIFY(logger.isEnabled());
        QVERIFY(logger.lastError().contains(QStringLiteral("owner thread")));
        QVERIFY(caller.flushError.contains(QStringLiteral("owner thread")));
        QVERIFY(caller.shutdownError.contains(QStringLiteral("owner thread")));
        QVERIFY(!logger.m_buffer.isEmpty());
    }

    void crossThreadDisableDoesNotChangeStateOrFlush()
    {
        MonitorDataLogger logger;
        logger.enqueueSample(makeSample(QStringLiteral("cross_thread.disable.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         5.0));

        CrossThreadDisableCaller caller(&logger);
        caller.start();
        QVERIFY(caller.wait(500));
        QVERIFY(logger.isEnabled());
        QVERIFY(!logger.m_buffer.isEmpty());
        QVERIFY(caller.error.contains(QStringLiteral("owner thread")));
    }

    void disablingWithFailedFlushKeepsLoggerEnabled()
    {
        MonitorDataLogger logger;
        logger.enqueueSample(makeSample(QStringLiteral("disable.failed.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         6.0));
        ...
        logger.enqueueSample(makeSample(QStringLiteral("disable.failed.retry.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         7.0));
        QCOMPARE(logger.m_buffer.size(), 2);
    }

    void shutdownDropsSamplesThatCannotBePersisted()
    {
        MonitorDataLogger logger;
        logger.enqueueSample(makeSample(QStringLiteral("shutdown.failed.point"),
                                         RuntimePointQuality::Good,
                                         true,
                                         6.0));
        ...
        QCOMPARE(logger.droppedSampleCount(), quint64(1));
        QVERIFY(logger.m_buffer.isEmpty());
        QVERIFY(logger.lastError().contains(QStringLiteral("shutdown discarded 1")));
    }
    ...
    void cleanupTimerPurgesPersistentDataWhenLoggingEnabled()
    {
        ...
        QVERIFY(manager.startDatabaseService(databasePath));
        ...
        manager.setDatabaseLoggingEnabled(true);
        QVERIFY(QMetaObject::invokeMethod(&manager, "onCleanupTimeout", Qt::DirectConnection));
        QTRY_COMPARE(dataManager.queryHistory(QStringLiteral("cleanup.runtime"), start, end).size(), 0);
