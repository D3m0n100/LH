...
bool writePublishedDownloadBundle(const QString& projectPath,
                                  const QByteArray& code,
                                  const QByteArray& profile,
                                  ProjectRuntimeConfig* config,
                                  QString* artifactPath)
{
    const QString generationId = QStringLiteral("generation-1");
    ...
    if (!config || !artifactPath || !QDir().mkpath(generationPath))
        return false;
    ...
    const QString codeChecksum = QString::fromLatin1(fixtureChecksum(code));
    const QString profileChecksum = QString::fromLatin1(fixtureChecksum(profile));
    const QString pointsChecksum = QString::fromLatin1(fixtureChecksum(points));
    QJsonObject checksums;
    checksums.insert(QStringLiteral("main.code"), codeChecksum);
    checksums.insert(QStringLiteral("download_profile.json"), profileChecksum);
    checksums.insert(QStringLiteral("runtime_points.json"), pointsChecksum);
    ...
    manifest.insert(QStringLiteral("codeChecksum"), codeChecksum);
    manifest.insert(QStringLiteral("downloadProfilePath"), QStringLiteral("download_profile.json"));
    manifest.insert(QStringLiteral("downloadProfileChecksum"), profileChecksum);
    manifest.insert(QStringLiteral("runtimePointsPath"), QStringLiteral("runtime_points.json"));
    manifest.insert(QStringLiteral("runtimePointsChecksum"), pointsChecksum);
    ...
    if (!writeFixtureFile(manifestPath, manifestBytes))
        return false;

    config->downloadArtifact.filePath = QStringLiteral(
            "build_output/configuration/generations/generation-1/main.code");
    config->downloadArtifact.checksum = codeChecksum;
    config->downloadArtifact.metadata.insert(QStringLiteral("generationId"), generationId);
    config->downloadArtifact.metadata.insert(QStringLiteral("runtimeManifestPath"), QStringLiteral(
            "build_output/configuration/generations/generation-1/runtime_manifest.json"));
    config->downloadArtifact.metadata.insert(QStringLiteral("downloadProfilePath"), QStringLiteral(
            "build_output/configuration/generations/generation-1/download_profile.json"));
    config->downloadArtifact.metadata.insert(QStringLiteral("downloadProfileChecksum"), profileChecksum);
    config->downloadArtifact.metadata.insert(QStringLiteral("runtimePointsChecksum"), pointsChecksum);
    config->downloadArtifact.metadata.insert(QStringLiteral("runtimeManifestChecksum"),
                                             QString::fromLatin1(fixtureChecksum(manifestBytes)));
    *artifactPath = codePath;
    return true;
}
} // namespace
...
public:
    ...
    bool readHoldingRegisters(int address, int count) override
    {
        ...
        readCalls.append({station, address, count});
        ...
        if (registers.value(station).contains(address)) {
            if (registers.value(station).value(address).size() >= count) {
                return true;
            }
        }
        for (int i = 0; i < count; ++i) {
            if (!registers.value(station).contains(address + i)) {
                return false;
            }
        }
        return true;
    ...
    {
        if (registers.value(station).contains(address)) {
            const auto& direct = registers.value(station).value(address);
            if (direct.size() >= lastReadCount) {
                return direct;
            }
        }
        QVector<quint16> assembled;
        for (int i = 0; i < lastReadCount; ++i) {
            if (registers.value(station).contains(address + i)) {
                const auto& vec = registers.value(station).value(address + i);
                if (!vec.isEmpty()) {
                    assembled.append(vec.first());
                }
            }
        }
        if (assembled.size() >= lastReadCount) {
            return assembled;
        }
...
private slots:
    ...
    void downloadArtifactRejectsMissingProfileBeforeWrite()
    {
        ...
        QVERIFY(!backend.downloadArtifact(artifact.fileName(),
                                          {{QStringLiteral("downloadProfilePath"), QStringLiteral("missing.json")}},
                                          &errorMessage,
                                          &operationError));
        QCOMPARE(operationError.code, CommErrorCode::InvalidConfig);
        QVERIFY(errorMessage.contains(QStringLiteral("generationId")));
        QVERIFY(transport.writes.isEmpty());

        errorMessage.clear();
        operationError = CommError();
        QVERIFY(!backend.downloadArtifact(artifact.fileName(),
                                          {{QStringLiteral("generationId"), QStringLiteral("test-generation")},
                                           {QStringLiteral("runtimeManifestPath"), QStringLiteral("test-generation/runtime_manifest.json")},
                                           {QStringLiteral("checksum"), QStringLiteral("test-checksum")}},
                                          &errorMessage,
                                          &operationError));
        QCOMPARE(operationError.code, CommErrorCode::InvalidConfig);
        QVERIFY(errorMessage.contains(QStringLiteral("generation")));
        QVERIFY(transport.writes.isEmpty());
        QCOMPARE(backend.statusSnapshot().downloadPercent, 0);
    }

    void downloadArtifactRejectsPublishedChecksumMismatchBeforeWrite()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        QString artifactPath;
        QVERIFY(writePublishedDownloadBundle(projectDir.path(),
                                             QByteArray::fromHex("12345678"),
                                             QByteArrayLiteral(R"({
            "name": "checksum",
            "slaveId": 1,
            "steps": [{"type": "sendChunk", "params": {"dataAddress": 210, "chunkWords": 2}}]
        })"),
                                             &cfg,
                                             &artifactPath));
        cfg.downloadArtifact.metadata.insert(QStringLiteral("downloadProfileChecksum"),
                                             QStringLiteral("forged-profile-checksum"));
        QVERIFY(backend.configure(cfg));
        QVERIFY(backend.connectBackend());

        QString errorMessage;
        CommError operationError;
        QVERIFY(!backend.downloadArtifact(artifactPath, {}, &errorMessage, &operationError));
        QCOMPARE(operationError.code, CommErrorCode::InvalidConfig);
    ...
    void downloadArtifactCanExecuteConfiguredProfile()
    {
        ...
        ProjectRuntimeConfig cfg;
        cfg.transport.parameters.insert(QStringLiteral("port"), QStringLiteral("COM7"));
        const QByteArray profile = QByteArrayLiteral(R"({
            "name": "test",
            "slaveId": 1,
            "steps": [
        ...
        QVERIFY(backend.downloadArtifact(artifactPath, {}));
    ...
    void validateOnlyDownloadDoesNotTouchTransport()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        QVERIFY(backend.downloadArtifact(artifact.fileName(),
                                         {{QStringLiteral("downloadProfilePath"), profile.fileName()},
                                          {QStringLiteral("validateOnly"), true}},
                                         &errorMessage));
    ...
    void downloadArtifactHonorsPreCancellation()
    {
        ControllerDeviceBackend backend;
        ProjectRuntimeConfig cfg;
        cfg.transport.parameters.insert(QStringLiteral("port"), QStringLiteral("COM7"));
        ...
        QString errorMessage;
        CommError operationError;
        const bool result = backend.downloadArtifact(artifact.fileName(),
                                                     {{QStringLiteral("dryRun"), true}},
                                                     &errorMessage,
                                                     &operationError);
        QVERIFY(!result);
        QCOMPARE(operationError.code, CommErrorCode::OperationCancelled);
        QVERIFY(errorMessage.contains(QStringLiteral("取消")));

        backend.resetDownloadCancellation();
        operationError = CommError();
        errorMessage.clear();
        backend.dryRunDownloadArtifact(artifact.fileName(), {}, &errorMessage);
        QVERIFY(!errorMessage.contains(QStringLiteral("取消")));
    }

    void downloadProfileValidationRejectsInvalidTimeoutAndInterval()
    {
        auto makePollProfile = [](const QVariant& timeoutVal, const QVariant& intervalVal) {
            DownloadProfile p;
            p.name = QStringLiteral("poll-val");
            p.steps = {
                {DownloadProfile::StepType::SendChunk,
                 {{QStringLiteral("dataAddress"), 20}, {QStringLiteral("chunkWords"), 1}}},
                {DownloadProfile::StepType::Poll,
                 {{QStringLiteral("address"), 300},
                  {QStringLiteral("count"), 1},
                  {QStringLiteral("expected"), QVariantList{1}},
                  {QStringLiteral("timeoutMs"), timeoutVal},
                  {QStringLiteral("pollIntervalMs"), intervalVal}}}
            };
            return p;
        };
        ...
        QStringList errs;
        // Bool timeout
        QVERIFY(!makePollProfile(true, 100).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("timeoutMs")));
        // Float timeout
        errs.clear();
        QVERIFY(!makePollProfile(100.5, 100).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("timeoutMs")));
        // Negative timeout
        errs.clear();
        QVERIFY(!makePollProfile(-10, 100).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("timeoutMs")));
        // Over-bound timeout (600001)
        errs.clear();
        QVERIFY(!makePollProfile(600001, 100).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("timeoutMs")));
        // Bool interval
        errs.clear();
        QVERIFY(!makePollProfile(1000, false).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("pollIntervalMs")));
        // Over-bound interval (60001)
        errs.clear();
        QVERIFY(!makePollProfile(1000, 60001).validate(&errs));
        QVERIFY(errs.join(QStringLiteral("; ")).contains(QStringLiteral("pollIntervalMs")));
        // Valid bounds
        errs.clear();
        QVERIFY(makePollProfile(10, 1).validate(&errs));
        errs.clear();
        QVERIFY(makePollProfile(600000, 60000).validate(&errs));
    }

    void downloadArtifactPollStepRespectsDeadlineNotInterval()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        // Register 300 returns 0, never matching expected 999
        transport.registers[1][300] = QVector<quint16>{0};

        QElapsedTimer timer;
        timer.start();
        QString errorMessage;
        CommError opError;
        const bool result = backend.downloadArtifact(artifactPath, {}, &errorMessage, &opError);
        const qint64 elapsed = timer.elapsed();

        QVERIFY(!result);
        QCOMPARE(opError.code, CommErrorCode::ReceiveTimeout);
        // Acceptance criteria: timeout=200ms, interval=5000ms must not wait 5 seconds.
        // Bounded by scheduling error, elapsed must be < 800ms.
        QVERIFY2(elapsed < 800, qPrintable(QStringLiteral("Elapsed time was %1 ms, expected < 800 ms").arg(elapsed)));
    }

    void downloadArtifactPollStepRespondsToCancellationPromptly()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        QElapsedTimer cancelElapsed;
        qint64 cancelToFinishMs = 0;
        QObject::connect(&cancelTimer, &QTimer::timeout, [&backend, &cancelElapsed]() {
            cancelElapsed.start();
            backend.cancelDownload();
        });

        QThread cancelThread;
        cancelTimer.moveToThread(&cancelThread);
        QObject::connect(&cancelThread, &QThread::started, [&cancelTimer]() {
            cancelTimer.start(60);
        });
        cancelThread.start();
        ...
        CommError opError;
        const bool result = backend.downloadArtifact(artifactPath, {}, &errorMessage, &opError);
        cancelToFinishMs = cancelElapsed.isValid() ? cancelElapsed.elapsed() : 9999;

        cancelThread.quit();
        cancelThread.wait();

        QVERIFY(!result);
        QCOMPARE(opError.code, CommErrorCode::OperationCancelled);
        // Acceptance criteria: UI waiting phase cancellation response target <= 250ms
        QVERIFY2(cancelToFinishMs <= 250,
                 qPrintable(QStringLiteral("Cancel response was %1 ms, expected <= 250 ms").arg(cancelToFinishMs)));
    }

    void readPointsBatchesContiguousRegisters()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        for (int i = 0; i < 100; ++i) {
            HardwareResourceBinding point;
            point.id = QStringLiteral("batch.pt.%1").arg(i);
            point.resourceName = point.id;
            point.resourceType = QStringLiteral("holding");
            point.channel = point.id;
            point.metadata.insert(QStringLiteral("address"), 500 + i);
            point.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
            point.metadata.insert(QStringLiteral("elementCount"), 1);
            cfg.resources.append(point);
            pointIds.append(point.id);

            transport.registers[1][500 + i] = QVector<quint16>{static_cast<quint16>(1000 + i)};
        }

        QVERIFY(backend.configure(cfg));
        QVERIFY(backend.connectBackend());

        transport.readCalls.clear();

        QHash<QString, QVariant> values;
        QVERIFY(backend.readPoints(pointIds, values));
        QCOMPARE(values.size(), 100);
        for (int i = 0; i < 100; ++i) {
            QCOMPARE(values.value(QStringLiteral("batch.pt.%1").arg(i)).toInt(), 1000 + i);
        }

        // 检查 holding 寄存器读取请求（address >= 500）
        QVector<FakeControllerTransport::ReadCall> batchCalls;
        for (const auto& call : transport.readCalls) {
            if (call.address >= 500) {
                batchCalls.append(call);
            }
        }
        // 100 个连续寄存器必须合并为 1 次 100 寄存器读取请求！
        QCOMPARE(batchCalls.size(), 1);
        QCOMPARE(batchCalls.first().station, 1);
        QCOMPARE(batchCalls.first().address, 500);
        QCOMPARE(batchCalls.first().count, 100);
    }

    void readPointsDoesNotMergeAcrossDevicesOrGaps()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        // Point A: dev 1, addr 600
        HardwareResourceBinding ptA;
        ptA.id = QStringLiteral("sep.dev1.a");
        ptA.resourceName = ptA.id;
        ptA.resourceType = QStringLiteral("holding");
        ptA.channel = ptA.id;
        ptA.metadata.insert(QStringLiteral("unitId"), 1);
        ptA.metadata.insert(QStringLiteral("address"), 600);
        ptA.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
        cfg.resources.append(ptA);

        // Point B: dev 2, addr 601（地址虽相邻，但不同设备）
        HardwareResourceBinding ptB;
        ptB.id = QStringLiteral("sep.dev2.b");
        ptB.resourceName = ptB.id;
        ptB.resourceType = QStringLiteral("holding");
        ptB.channel = ptB.id;
        ptB.metadata.insert(QStringLiteral("unitId"), 2);
        ptB.metadata.insert(QStringLiteral("address"), 601);
        ptB.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
        cfg.resources.append(ptB);

        // Point C: dev 1, addr 610（同为 dev 1，但 600 与 610 之间存在地址空洞）
        HardwareResourceBinding ptC;
        ptC.id = QStringLiteral("sep.dev1.c");
        ptC.resourceName = ptC.id;
        ptC.resourceType = QStringLiteral("holding");
        ptC.channel = ptC.id;
        ptC.metadata.insert(QStringLiteral("unitId"), 1);
        ptC.metadata.insert(QStringLiteral("address"), 610);
        ptC.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
        cfg.resources.append(ptC);

        transport.registers[1][600] = {11};
        transport.registers[2][601] = {22};
        transport.registers[1][610] = {33};
        ...
        QHash<QString, QVariant> values;
        QVERIFY(backend.readPoints({ptA.id, ptB.id, ptC.id}, values));
        QCOMPARE(values.value(ptA.id).toInt(), 11);
        QCOMPARE(values.value(ptB.id).toInt(), 22);
        QCOMPARE(values.value(ptC.id).toInt(), 33);

        QVector<FakeControllerTransport::ReadCall> customCalls;
        for (const auto& call : transport.readCalls) {
            if (call.address >= 600) {
                customCalls.append(call);
            }
        }
        // 绝不跨设备合并，绝不跨空洞合并：产生恰好 3 次读取！
        QCOMPARE(customCalls.size(), 3);
    }

    void readPointsSplitsWhenExceedingMaxBatchLimit()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        QStringList pointIds;
        // 150 个连续寄存器点位：700..849
        for (int i = 0; i < 150; ++i) {
            HardwareResourceBinding point;
            point.id = QStringLiteral("over125.pt.%1").arg(i);
            point.resourceName = point.id;
            point.resourceType = QStringLiteral("holding");
            point.channel = point.id;
            point.metadata.insert(QStringLiteral("address"), 700 + i);
            point.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
            point.metadata.insert(QStringLiteral("elementCount"), 1);
            cfg.resources.append(point);
            pointIds.append(point.id);

            transport.registers[1][700 + i] = QVector<quint16>{static_cast<quint16>(2000 + i)};
        }
        ...
        // 150 个点位必须被拆分成 2 批（125 + 25）
        QCOMPARE(batchCalls.size(), 2);
        QCOMPARE(batchCalls.at(0).address, 700);
        QCOMPARE(batchCalls.at(0).count, 125);
        QCOMPARE(batchCalls.at(1).address, 825);
        QCOMPARE(batchCalls.at(1).count, 25);
    }

    void readPointsPartialBatchFailurePreservesOtherBatches()
    {
        FakeControllerTransport transport;
        ControllerDebugClient client(&transport);
        ControllerDeviceBackend backend;
        ...
        // Batch 1 位于 900
        HardwareResourceBinding ptFail;
        ptFail.id = QStringLiteral("fail.pt");
        ptFail.resourceName = ptFail.id;
        ptFail.resourceType = QStringLiteral("holding");
        ptFail.channel = ptFail.id;
        ptFail.metadata.insert(QStringLiteral("address"), 900);
        ptFail.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
        cfg.resources.append(ptFail);

        // Batch 2 位于 950
        HardwareResourceBinding ptOk;
        ptOk.id = QStringLiteral("ok.pt");
        ptOk.resourceName = ptOk.id;
        ptOk.resourceType = QStringLiteral("holding");
        ptOk.channel = ptOk.id;
        ptOk.metadata.insert(QStringLiteral("address"), 950);
        ptOk.metadata.insert(QStringLiteral("dataType"), QStringLiteral("UINT"));
        cfg.resources.append(ptOk);

        transport.registers[1][900] = {99};
        transport.registers[1][950] = {100};
        transport.failReadAddress = 900; // 注入 900 地址失败
        ...
        QHash<QString, CommError> pointErrors;
        QString errorMessage;
        const bool result = backend.readPoints({ptFail.id, ptOk.id}, values, &errorMessage, &pointErrors);

        QVERIFY(!result);
        // Batch 2 成功的结果不受影响，不被抹除
        QVERIFY(values.contains(ptOk.id));
        QCOMPARE(values.value(ptOk.id).toInt(), 100);
        // 失败的批次保留准确的 pointError
        QVERIFY(pointErrors.contains(ptFail.id));
        QCOMPARE(pointErrors.value(ptFail.id).code, CommErrorCode::ReceiveFailed);
    }
};
