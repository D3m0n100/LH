...
private slots:
    ...
    void emptyDatabaseSnapshotExcludesSubsequentInserts()
    {
        QTemporaryDir tempDir;
        QVERIFY(tempDir.isValid());

        DataManager& manager = DataManager::instance();
        QVERIFY(manager.initialize(tempDir.filePath(QStringLiteral("empty_snapshot.db"))));

        // 此时数据库为空，获取导出快照 ID 为 0
        const qint64 snapshotMaxId = manager.latestRecordId();
        QCOMPARE(snapshotMaxId, qint64(0));

        RuntimeHistoryCursor cursor;
        cursor.maxId = snapshotMaxId; // maxId = 0

        // 模拟并发插入新记录（其 ID 会 >= 1）
        const QDateTime now = QDateTime::currentDateTimeUtc();
        QVERIFY(manager.logRuntimeData(QStringLiteral("pressure"), 42.0).success);
        QVERIFY(manager.latestRecordId() > 0);

        // 使用快照游标查询普通历史分页，必须严格查不到新插入记录
        const RuntimeHistoryPage page = manager.queryHistoryPage(
        ...
        const RuntimeHistoryCount count = manager.countHistory(
            ...
            snapshotMaxId);
        QCOMPARE(count.status, RuntimeHistoryPageStatus::Success);
        QCOMPARE(count.count, qint64(0));

        // 最近历史分页同样不能泄漏
        const RuntimeHistoryPage latestPage = manager.queryLatestHistoryPage(
            QStringLiteral("pressure"),
            10,
            10,
            cursor);
        QCOMPARE(latestPage.status, RuntimeHistoryPageStatus::Success);
        QVERIFY(latestPage.records.isEmpty());
        QVERIFY(!latestPage.hasMore);
