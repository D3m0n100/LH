...
void MonitorWidget::onExportData()
{
    ...
    if (fromDatabase) {
        ...
        auto* worker = manager.databaseWorker();
        if (!worker || !worker->isDbOpen()) {
        ...
        connect(this, &QObject::destroyed, [cancelled]() { cancelled->store(true); });
        progress->show();
        QPointer<MonitorWidget> guard(this);
        QPointer<QProgressDialog> progressGuard(progress);
        const bool accepted = worker->submitHistoryTask(this, [=]() mutable {
            const auto deliver = [=](ExportResult result) {
                if (!guard) return;
                QMetaObject::invokeMethod(guard, [=]() {
                    if (progressGuard) progressGuard->close();
                    if (!guard || cancelled->load()) return;
        if (result.success) {
            ...
            QMessageBox::information(guard, tr("导出成功"), message);
            qDebug() << "[MonitorWidget] 数据库流式导出成功:" << result.filePath;
        } else {
            QMessageBox::critical(guard, tr("导出失败"),
        ...
        for (int channelIndex = 0; channelIndex < channelInfos.size(); ++channelIndex) {
            ...
            RuntimeHistoryCount count = store->countHistory(
                channelId, fixedStart, fixedEnd, snapshotMaxId);
            if (!count.succeeded()) {
            ...
            if (selectedCount == 0) {
                state.fallback = true;
                RuntimeHistoryCount latestCount = store->countLatestHistory(
                    channelId, fallbackLimits.value(channelId), fixedEnd, snapshotMaxId);
        ...
        ExportPageProvider provider =
            [&, fixedStart, fixedEnd, snapshotMaxId](const QString& channelId,
                                                     const ExportCursor& cursor,
                                                     int requestedPageSize) -> ExportPage {
                ExportPage output;
                if (cancelled->load()) { output.success = false; output.errorMessage = QStringLiteral("Export cancelled"); return output; }
                ...
                historyCursor.maxId = snapshotMaxId;

                const int effectivePageSize = qMax(1, qMin(pageSize, requestedPageSize));
                Monitor::DatabaseHistoryPage page;
                if (state.fallback) {
                    page = history.page(channelId, {}, fixedEnd, effectivePageSize, historyCursor, fallbackLimits.value(channelId));
                } else {
                    page = history.page(
        ...
        ExportResult result = helper.exportPackagePaged(
        ...
        if (!accepted) {
            progress->close();
            QMessageBox::critical(this, tr("导出失败"), tr("历史请求队列已满或服务正在停止，请稍后重试"));
        }
        return;
    }
