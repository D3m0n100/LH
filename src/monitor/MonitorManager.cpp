...
MonitorManager::~MonitorManager()
{
    shutdown();
    if (m_historyStore) {
        m_historyStore->cancelPendingRequests();
    }
    m_dataLogger.reset();
}
...
void MonitorManager::shutdown()
{
    ...
    if (m_databaseWorker && !m_databaseWorker->stopWorker()) {
        qWarning() << "Database shutdown exceeded drain budget; connection cleanup deferred to SQL owner thread";
        m_dataLogger->setAsyncWorker(nullptr);
        m_databaseWorker.release()->deleteWhenStopped();
    }
}

bool MonitorManager::startDatabaseService(const QString& databasePath)
{
    if (m_databaseWorker && m_databaseWorker->isDbOpen()) return true;
    auto worker = std::make_unique<Core::AsyncDatabaseWorker>();
    if (!worker->startWorker(databasePath)) return false;
    m_dataLogger->setAsyncWorker(worker.get());
    setHistoryStore(std::make_shared<AsyncHistoryStoreAdapter>(worker.get()));
    connect(worker.get(), &Core::AsyncDatabaseWorker::workerError, this,
            [](const QString& operation, const QString& error) { qWarning() << operation << error; });
    connect(worker.get(), &Core::AsyncDatabaseWorker::cleanupFailed, this,
            [](const QString& error) { qWarning() << "Database cleanup failed:" << error; });
    m_databaseWorker = std::move(worker);
    return true;
}

void MonitorManager::startDatabaseServiceAsync(const QString& databasePath)
{
    if (m_databaseServiceStarting) return;
    if (m_databaseWorker) {
        emit databaseServiceStarted(m_databaseWorker->isDbOpen(), QStringLiteral("Database service already exists"));
        return;
    }
    const auto generation = ++m_databaseServiceGeneration;
    m_databaseServiceStarting = true;
    auto worker = std::make_unique<Core::AsyncDatabaseWorker>();
    auto* source = worker.get();
    const auto finished = std::make_shared<bool>(false);
    auto complete = [this, generation, source, finished](bool ok, const QString& error) {
        if (*finished || generation != m_databaseServiceGeneration) return;
        *finished = true;
        m_databaseServiceStarting = false;
        if (ok) {
            m_dataLogger->setAsyncWorker(source);
            setHistoryStore(std::make_shared<AsyncHistoryStoreAdapter>(source));
        } else if (m_databaseWorker.get() == source) {
            auto* abandoned = m_databaseWorker.release();
            abandoned->stopWorker(0);
            abandoned->deleteWhenStopped();
        }
        emit databaseServiceStarted(ok, error);
    };
    connect(source, &Core::AsyncDatabaseWorker::startupFinished, this, complete);
    connect(source, &Core::AsyncDatabaseWorker::workerError, this,
            [](const QString& operation, const QString& error) { qWarning() << operation << error; });
    connect(source, &Core::AsyncDatabaseWorker::cleanupFailed, this,
            [](const QString& error) { qWarning() << "Database cleanup failed:" << error; });
    m_databaseWorker = std::move(worker);
    if (!source->startWorkerAsync(databasePath)) {
        complete(false, QStringLiteral("Cannot start database worker"));
        return;
    }
    QTimer::singleShot(3000, this, [complete]() {
        complete(false, QStringLiteral("Database startup deadline exceeded"));
    });
}

void MonitorManager::requestDatabaseHistoryPage(QObject* context, const QString& channel,
    const QDateTime& start, const QDateTime& end, int size, const RuntimeHistoryCursor& cursor,
    int latestCount, std::function<void(DatabaseHistoryPage)> completed)
{
    MonitorHistoryService(m_historyStore).requestPage(context, channel, start, end, size, cursor,
                                                    latestCount, std::move(completed));
}
...
void MonitorManager::setHistoryStore(std::shared_ptr<IMonitorHistoryStore> store)
{
...
std::shared_ptr<IMonitorHistoryStore> MonitorManager::historyStore() const
{
...
{
    return m_historyStore ? m_historyStore->isAvailable() : false;
}
...
void MonitorManager::onCleanupTimeout()
{
    ...
    if (m_databaseWorker) m_databaseWorker->requestCleanup(m_dataRetentionDays);
...
void MonitorManager::connectChannelSignals(const std::shared_ptr<MonitorChannel>& channel)
{
    connect(channel.get(), &MonitorChannel::thresholdExceeded,
            this, [this](const QString& channelName, double value,
                ...
                int suppressed = 0;
                {
                    ...
                    auto& rl = m_alarmRateLimits[channelName];
                    if (rl.lastLogTimeUtcMs == 0 || nowMs - rl.lastLogTimeUtcMs >= 2000) {
                        shouldLog = true;
                        suppressed = rl.suppressedCount;
                        rl.suppressedCount = 0;
                        rl.lastLogTimeUtcMs = nowMs;
                    } else {
                        ++rl.suppressedCount;
                    }
                }
                ...
                if (shouldLog) {
                    ...
                    fields.insert(QStringLiteral("threshold"), thresholdValue);
                    fields.insert(QStringLiteral("unit"), unit);
                    fields.insert(QStringLiteral("mode"),
                                  mode == ThresholdMode::Above ? QStringLiteral("Above") : QStringLiteral("Below"));
                    if (suppressed > 0) {
                        fields.insert(QStringLiteral("suppressed"), suppressed);
                    }
                    AppLogging::writeBusinessEvent(QStringLiteral("alarm_triggered"),
                                                   QtWarningMsg,
                                                   QString(),
                                                   QStringLiteral("monitor_alarm"),
    ...
    connect(channel.get(), &MonitorChannel::thresholdCleared,
            this, [this](const QString& channelName, double value,
                         const QString& unit, double thresholdValue, ThresholdMode mode) {
                {
                    QMutexLocker locker(&m_alarmMutex);
                    m_alarmRateLimits.remove(channelName);
                }
                ...
                fields.insert(QStringLiteral("mode"),
                              mode == ThresholdMode::Above ? QStringLiteral("Above") : QStringLiteral("Below"));
                AppLogging::writeBusinessEvent(QStringLiteral("alarm_cleared"),
                                               QtInfoMsg,
                                               QString(),
                                               QStringLiteral("monitor_alarm"),
                ...
                emit thresholdCleared(channelName, value, thresholdValue);
            });
}
