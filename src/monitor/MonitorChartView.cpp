...
// 默认颜色列表
const QList<QColor> MonitorChartView::DEFAULT_COLORS = Monitor::defaultChannelColors();
...
{
    return SvgExport::write(m_chartWidget, filePath);
}
...
void MonitorChartView::doRender()
{
    ...
    for (auto it = m_pendingDeltas.begin(); it != m_pendingDeltas.end(); ++it) {
        ...
        // 增量追加数据点（批量提交）
        m_chartWidget->appendPoints(channelId, points);
