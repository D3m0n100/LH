...
QVariant MatrikonOpcServer::variantToQVariant(const void* variant)
{
    ...
    case VT_LPSTR: {
        const auto* value = reinterpret_cast<const char*>(v->byref);
        return value ? QString::fromLocal8Bit(value) : QVariant();
    }
    case VT_LPWSTR: {
        const auto* value = reinterpret_cast<const wchar_t*>(v->byref);
        return value ? QString::fromWCharArray(value) : QVariant();
    }
    default:
        return {};
    }
