...
{
    if (m_requestCancelled && m_requestCancelled->load()) return CommErrorCode::OperationCancelled;
    if (guardTimedOut) {
...
void ModbusInterface::setRequestBudget(int remainingMs, const std::atomic_bool* cancelled)
{
    m_requestDeadline = remainingMs < 0 ? QDeadlineTimer(QDeadlineTimer::Forever) : QDeadlineTimer(remainingMs, Qt::PreciseTimer);
    m_requestCancelled = cancelled;
    if (m_client) {
        if (remainingMs >= 0) {
            m_client->setTimeout(qMax(1, qMin(m_config.responseTimeout, remainingMs)));
            m_client->setNumberOfRetries(0); // Outer step owns the remaining deadline.
        } else applyTimeoutRetry();
    }
}
...
bool ModbusInterface::waitForReply(QModbusReply* reply, QString& outErr, bool& guardTimedOut)
{
    ...
    if (m_requestDeadline.hasExpired() || (m_requestCancelled && m_requestCancelled->load())) {
        guardTimedOut = true; outErr = QStringLiteral("Request deadline expired or cancelled"); return false;
    }
    if (reply->isFinished()) {
    ...
    // 超时裕量：timeout * attempts + 200ms，带溢出防护和上限约束（最大 60000ms）
    const int attempts = qBound(1, m_config.retryCount, 10);
    const qint64 rawWait = static_cast<qint64>(qMax(50, m_config.responseTimeout)) * attempts + 200;
    const int waitMs = static_cast<int>(qMin<qint64>(qBound<qint64>(50, rawWait, 60000),
        m_requestDeadline.isForever() ? 60000 : qMax<qint64>(0, m_requestDeadline.remainingTime())));
    QTimer cancellationTimer;
    cancellationTimer.setInterval(10);
    connect(&cancellationTimer, &QTimer::timeout, &loop, [&]() {
        if (m_requestCancelled && m_requestCancelled->load()) { guardTimedOut = true; loop.quit(); }
    });
    cancellationTimer.start();
    timer.start(waitMs);

    const QMetaObject::Connection connLost = connect(this, &ModbusInterface::connectionStateChanged,
                                                     &loop, [&loop](bool connected) {
        if (!connected) {
            loop.quit();
        }
    });
    ...
    if (!reply->isFinished()) guardTimedOut = true;
    if (guardTimedOut) {
...
bool ModbusInterface::readDataUnit(const QModbusDataUnit& unit, int serverAddress)
{
    ...
    if (m_requestDeadline.hasExpired() || (m_requestCancelled && m_requestCancelled->load())) {
        reportError(m_requestCancelled && m_requestCancelled->load() ? CommErrorCode::OperationCancelled : CommErrorCode::ReceiveTimeout,
                    QStringLiteral("Request cancelled or deadline expired")); return false;
    }
