...
ControllerDebugClient::~ControllerDebugClient()
{
    if (m_actor) {
        QMetaObject::invokeMethod(m_actor, [this]() { delete m_actor; m_actor = nullptr; }, Qt::BlockingQueuedConnection);
        m_actorThread->quit(); m_actorThread->wait(); delete m_actorThread;
    }
}

void ControllerDebugClient::enableWorkerThread(std::function<IControllerDebugTransport*()> factory)
{
    if (m_actor) return;
    m_actorThread = new QThread();
    auto* bootstrap = new QObject();
    bootstrap->moveToThread(m_actorThread);
    m_actorThread->start();
...
void ControllerDebugClient::runAsync(std::function<void()> job)
{
    Q_ASSERT(m_actor);
    QMetaObject::invokeMethod(m_actor, std::move(job), Qt::QueuedConnection);
}
void ControllerDebugClient::waitForIdle()
{
    if (m_actor) dispatch([]() { return true; });
}
...
{
    if (m_actor) { dispatch([&]() { m_actor->clearError(); return true; }); return; }
    m_lastError = {};
}
void ControllerDebugClient::setRequestBudget(int remainingMs, const std::atomic_bool* cancelled)
{
    if (m_actor) { dispatch([&]() { m_actor->setRequestBudget(remainingMs, cancelled); return true; }); return; }
    if (m_transport) m_transport->setRequestBudget(remainingMs, cancelled);
}
void ModbusControllerDebugTransport::setRequestBudget(int remainingMs, const std::atomic_bool* cancelled)
{
    if (m_modbus) m_modbus->setRequestBudget(remainingMs, cancelled);
}
...
{
    if (m_actor) return dispatch([&]() { return m_actor->openRtu(portName, deviceId); });
    ...
    m_connectionSnapshot = true;
    connect(modbus.get(), &ModbusInterface::connectionStateChanged, this,
            [this](bool connected) { m_connectionSnapshot = connected; });
...
{
    if (m_actor) return dispatch([&]() { return m_actor->openRtu(config, deviceId); });
    if (!ControllerDebugProtocol::canReadDeviceId(deviceId)) {
        return fail(CommErrorCode::InvalidAddress,
                    QStringLiteral("Controller read/debug connection requires device id 1..63"),
                    QStringLiteral("deviceId=%1").arg(deviceId));
...
{
    if (m_actor) { dispatch([&]() { m_actor->close(); return true; }); return; }
    m_connectionSnapshot = false;
    if (m_ownedModbus) {
        m_ownedModbus->close();
    }
    m_ownedTransport.reset();
...
{
    if (m_actor) return m_actor->m_connectionSnapshot.load();
...
{
    if (m_actor) return dispatch([&]() { return m_actor->readRegister(reg, value); });
    if (!value) {
...
{
    if (m_actor) return dispatch([&]() { return m_actor->readHoldingRegisters(deviceId, address, count, values); });
    if (!values) {
        return fail(CommErrorCode::InvalidParameter, QStringLiteral("Controller read target is null"));
    }
    values->clear();
...
{
    if (m_actor) return dispatch([&]() { return m_actor->writeHoldingRegisters(deviceId, address, values); });
    if (!ensureTransport()) {
        return false;
    }
    if (!ControllerDebugProtocol::canReadDeviceId(deviceId)) {
...
{
    if (m_actor) return dispatch([&]() { return m_actor->readStatus(status); });
    if (!status) {
