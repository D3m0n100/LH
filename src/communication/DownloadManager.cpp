...
public slots:
    ...
    void doConnectProbe()
    {
        ...
        const QString stage = iface->getParameter("bridgeStage").toString();
        if (stage == QStringLiteral("controllerOffline") || (!stage.isEmpty() && !iface->getParameter("controllerOnline").toBool())) {
        ...
        if (stage == QStringLiteral("targetOffline") || (!stage.isEmpty() && !iface->getParameter("targetOnline").toBool())) {
            const CommError last = iface->lastError();
