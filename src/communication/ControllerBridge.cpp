...
bool ControllerBridge::stepPoll(const QVariantMap& params)
{
    ...
    const int timeoutMs = qBound(10, params.value("timeoutMs", 2000).toInt(), 600000);
    const int pollMs = qBound(1, params.value("pollIntervalMs", 100).toInt(), 60000);
    ...
    struct ResetBudget { ModbusInterface* modbus; const std::atomic_bool* cancelled; ~ResetBudget() { modbus->setRequestBudget(-1, cancelled); } } resetBudget{m_modbus, &m_abortRequested};
    QElapsedTimer t;
    t.start();
    while (true) {
        if (isAbortRequested()) {
        ...
        m_modbus->setRequestBudget(qMax(0, timeoutMs - static_cast<int>(t.elapsed())), &m_abortRequested);
        if (m_modbus->readHoldingRegisters(addr, count)) {
            if (t.elapsed() >= timeoutMs || isAbortRequested()) break;
            const auto got = m_modbus->holdingRegisters().value(addr);
            if (got.size() >= count) {
                bool match = true;
        ...
        const qint64 remaining = timeoutMs - t.elapsed();
        if (remaining <= 0) {
            break;
        }
        const int waitTargetMs = static_cast<int>(qMin<qint64>(pollMs, remaining));
...
bool ControllerBridge::download(const DownloadProfile& profile, const QByteArray& payload)
{
    ...
    m_modbus->setRequestBudget(-1, &m_abortRequested);
    struct ResetDownloadBudget { ModbusInterface* modbus; ~ResetDownloadBudget() { modbus->setRequestBudget(-1, nullptr); } } resetDownloadBudget{m_modbus};
