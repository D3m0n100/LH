...
bool strictBoundedInt(const QVariant& value, int minimum, int maximum, int* out)
{
    if (value.type() == QVariant::Bool) return false;
    bool ok = false;
    const double number = value.toDouble(&ok);
    if (!ok || !qIsFinite(number) || qFloor(number) != number || number < minimum || number > maximum) {
        return false;
    }
    if (out) *out = static_cast<int>(number);
    return true;
...
bool DownloadProfile::validate(QStringList* errors) const
{
    ...
    for (int i = 0; i < steps.size(); ++i) {
        ...
        case StepType::Poll: {
            ...
            int timeoutMs = 0;
            if (!strictBoundedInt(params.value(QStringLiteral("timeoutMs"), 2000), 10, 600000, &timeoutMs)) {
                stepErrors << QStringLiteral("timeoutMs 必须为 10..600000 ms 的有效整数");
            }
            int pollIntervalMs = 0;
            if (!strictBoundedInt(params.value(QStringLiteral("pollIntervalMs"), 100), 1, 60000, &pollIntervalMs)) {
