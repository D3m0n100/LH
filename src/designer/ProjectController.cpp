...
bool ProjectController::readScriptBackup(const QString& path, QByteArray* bytes, QString* error)
{
    QFile file(path);
    if (!file.open(QIODevice::ReadOnly) || !FileSnapshot::readComplete(file, bytes)) {
...
bool ProjectController::saveProject()
{
    ...
    if (scriptExistedBefore) {
        QString backupError;
        if (!readScriptBackup(m_currentScriptFile, &previousScriptBytes, &backupError)) {
...
bool ProjectController::checkDuplicateChannelBindings(const ProjectRuntimeConfig& cfg, QStringList& errors) const
{
    ...
    // signalPath is descriptive routing metadata, not an exclusive resource key.
    // Inserted snippets use category.name, so multiple instances (and multiple
    // observers of one signal) legitimately share it. Mapping/provider IDs and
    // hardware resource ownership are validated separately.
