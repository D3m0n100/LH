...
void RuntimeSessionController::startOpcServerIfEnabled()
{
    ...
    if (!m_opcServer->applyConfig(opcConfig, &errorMessage)) {
        AppLogging::writeBusinessEvent(
            QStringLiteral("opc_server_start_failed"),
            QtCriticalMsg,
            QString(),
            QStringLiteral("opc"),
    ...
    AppLogging::writeBusinessEvent(
        ...
        QString(),
        QStringLiteral("opc"),
        (isDegraded ? QStringLiteral("degraded") : QStringLiteral("operational")),
        0,
        opcConfig.opcProgId,
        {{QStringLiteral("channel"), opcConfig.channelName},
         {QStringLiteral("device"), opcConfig.deviceName},
         {QStringLiteral("mode"), opcConfig.serialMode}});

    if (isDegraded) {
        emit logMessage(QStringLiteral("[%1] [警告] OPC DA 服务已启动，但处于降级状态 (degraded)：%2")
                        .arg(QDateTime::currentDateTime().toString(QStringLiteral("HH:mm:ss")),
                             status.lastErrorDetails.isEmpty() ? status.lastErrorMessage : status.lastErrorDetails));
    } else {
        emit logMessage(QStringLiteral("[%1] OPC DA 服务已就绪并正常运行 (operational)：progId=%2 channel=%3 device=%4 mode=%5")
                        .arg(QDateTime::currentDateTime().toString(QStringLiteral("HH:mm:ss")),
                             opcConfig.opcProgId,
...
void RuntimeSessionController::stopOpcServer()
{
    ...
    m_opcServer->stop();
    AppLogging::writeBusinessEvent(
        QStringLiteral("opc_server_stopped"),
        QtInfoMsg,
        QString(),
        QStringLiteral("opc"),
        QStringLiteral("stopped"),
        0,
        m_projectController ? m_projectController->runtimeConfig().opcServer.opcProgId : QStringLiteral("OpcServer"));
...
void RuntimeSessionController::handleOpcWriteRequest(const QString& pointId, const QVariant& value)
{
    ...
    m_pendingOpcOperationId = QStringLiteral("op-opc-%1-%2")
        .arg(QDateTime::currentMSecsSinceEpoch())
        .arg(pointId);

    AppLogging::writeBusinessEvent(
        QStringLiteral("opc_write_received"),
        QtInfoMsg,
        m_pendingOpcOperationId,
        QStringLiteral("opc_write"),
        QStringLiteral("received"),
        0,
        pointId,
        {{QStringLiteral("parameter"), stateInfo.name},
    ...
    if (!ok) {
        ...
        AppLogging::writeBusinessEvent(
            QStringLiteral("opc_write_failed"),
            QtWarningMsg,
            m_pendingOpcOperationId,
            QStringLiteral("opc_write"),
                                                                ...
                                                                const QString& message)
{
    ...
    AppLogging::writeBusinessEvent(
        confirmed ? QStringLiteral("opc_write_succeeded") : QStringLiteral("opc_write_failed"),
        confirmed ? QtInfoMsg : QtWarningMsg,
        m_pendingOpcOperationId,
        QStringLiteral("opc_write"),
        confirmed ? QStringLiteral("confirmed") : QStringLiteral("failed"),
        confirmed ? 0 : -1,
        pointId,
        {{QStringLiteral("parameter"), parameterName},
...
void RuntimeSessionController::cancelPendingOpcWrite(const QString& message)
{
    ...
    AppLogging::writeBusinessEvent(
        QStringLiteral("opc_write_canceled"),
        QtWarningMsg,
        m_pendingOpcOperationId,
        QStringLiteral("opc_write"),
