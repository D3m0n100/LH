                         ...
                         QVariantMap* details)
{
    ...
    const QJsonArray scriptFiles = manifest.value(QStringLiteral("scriptFiles")).toArray();
    const QJsonObject sourceChecksums = manifest.value(QStringLiteral("sourceChecksums")).toObject();

    if (projectRoot.isEmpty()) {
        if (errors)
            errors->append(QStringLiteral("projectRoot 为空，无法校验源码一致性。"));
        return false;
    }
    if (scriptFiles.isEmpty()) {
        if (errors)
            errors->append(QStringLiteral("manifest 中 scriptFiles 为空或缺失。"));
        return false;
    }
    if (sourceChecksums.size() != scriptFiles.size()) {
        if (errors)
            errors->append(QStringLiteral("manifest sourceChecksums 与 scriptFiles 数量不一致（scriptFiles: %1, sourceChecksums: %2）。")
                           .arg(scriptFiles.size()).arg(sourceChecksums.size()));
        return false;
    }

    QSet<QString> seenScripts;
    for (const QJsonValue& val : scriptFiles) {
        const QString relative = val.toString().trimmed();
        if (relative.isEmpty()) {
            if (errors)
                errors->append(QStringLiteral("manifest scriptFiles 包含无效或空路径。"));
            return false;
        }
        if (seenScripts.contains(relative)) {
            if (errors)
                errors->append(QStringLiteral("manifest scriptFiles 包含重复路径：%1").arg(relative));
            return false;
        }
        seenScripts.insert(relative);

        if (!sourceChecksums.contains(relative)) {
            if (errors)
                errors->append(QStringLiteral("manifest sourceChecksums 缺失源码文件项：%1").arg(relative));
            return false;
        }
        const QString expectedHash = sourceChecksums.value(relative).toString().trimmed();
        if (expectedHash.isEmpty()) {
            if (errors)
                errors->append(QStringLiteral("manifest 源码 checksum 缺失或为空：%1").arg(relative));
            return false;
        }
        QString resolvedSource;
        if (!resolveContainedPath(projectRoot, relative, &resolvedSource, &error, false)
                || !QFileInfo::exists(resolvedSource)) {
            if (errors)
                errors->append(QStringLiteral("源文件不存在或无法访问：%1").arg(relative));
            return false;
        }
        const QString currentHash = sha256ForFile(resolvedSource);
        if (currentHash.isEmpty() || currentHash.compare(expectedHash, Qt::CaseInsensitive) != 0) {
            if (errors)
                errors->append(QStringLiteral("源码已修改（%1 checksum 不一致），旧 generation 已失效，请重新编译。").arg(relative));
            return false;
        }
    }
...
void appendRequiredPublishedBindingErrors(const ProjectRuntimeConfig& config,
                                          RunController::DownloadArtifactPrecheckReport* report)
{
    if (!report)
        return;

    if (metadataPath(config, QStringLiteral("generationId")).isEmpty())
        report->errors << QStringLiteral("物理控制器下载要求产物绑定到已发布 generation。缺少 generationId。");
    if (firstMetadataPath(config, {QStringLiteral("runtimeManifestPath"),
                                   QStringLiteral("manifestPath")}).isEmpty())
        report->errors << QStringLiteral("物理控制器下载要求已发布 generation manifest。缺少 runtime_manifest 路径。");
    if (config.downloadArtifact.checksum.trimmed().isEmpty())
        report->errors << QStringLiteral("物理控制器下载要求产物 checksum。缺少 code checksum。");
    ...
    for (const QString& key : requiredChecksums) {
        if (metadataPath(config, key).isEmpty())
            report->errors << QStringLiteral("物理控制器下载要求已发布 artifact checksum。缺少 %1。")
                                  .arg(key);
    }
}
...
bool RunController::writeDownloadArtifact(ProjectRuntimeConfig& config,
                                          ...
                                          const CompileResult& compileResult)
{
    if (!compileResult.success || compileResult.metadata.value(QStringLiteral("compileOnly")).toBool())
        return false;
...
RunController::DownloadArtifactPrecheckReport RunController::validateDownloadArtifact(
        ...
        const QString& projectPath,
        const QString& artifactPath,
        bool requirePublishedBinding)
{
    const bool publishedBinding = !metadataPath(config, QStringLiteral("generationId")).isEmpty()
            || !metadataPath(config, QStringLiteral("runtimeManifestPath")).isEmpty();
    if (!publishedBinding) {
        DownloadArtifactPrecheckReport report = validateLegacyDownloadArtifact(
                config, projectPath, artifactPath);
        if (requirePublishedBinding)
            appendRequiredPublishedBindingErrors(config, &report);
        report.valid = report.errors.isEmpty();
        report.details.insert(QStringLiteral("valid"), report.valid);
        report.details.insert(QStringLiteral("errors"), stringListToVariantList(report.errors));
        return report;
    }
