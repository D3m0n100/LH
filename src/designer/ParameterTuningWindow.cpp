                                                                              ...
                                                                              const QString& groupKey) const
{
    ...
    for (const auto& parameter : parameters) {
        if (!parameter.onlineEditable || !PidParameterUtils::isPidParameter(parameter)) {
            continue;
        }
...
{
    const quint64 generation = ++m_pidHistoryGeneration;
    ...
    const auto render = [this, parameter = *it](const QList<Monitor::Sample>& samples, const QString& historyError) {
        m_pidSeries->clear();
        if (m_pidDefaultLine) {
            m_pidDefaultLine->clear();
        }
        ...
        const QString unitText = parameter.unit.trimmed().isEmpty() ? QString() : QStringLiteral(" (%1)").arg(parameter.unit);
        m_pidChart->setTitle(QStringLiteral("%1 参数趋势%2").arg(parameter.name, unitText));

        QString summary = QStringLiteral("参数：%1").arg(parameter.name);
        if (!parameter.currentValue.isEmpty()) {
            summary += QStringLiteral(" | 当前值：%1").arg(parameter.currentValue);
        }
        if (!parameter.defaultValue.isEmpty()) {
            summary += QStringLiteral(" | 默认值：%1").arg(parameter.defaultValue);
        }
        ...
        const bool hasReasonableRange = computeReasonableRange(parameter, reasonableMin, reasonableMax);
        ...
        if (samples.isEmpty()) {
            ...
            if (hasReasonableRange) {
                ...
                bool ok = false;
                const double center = parameter.currentValue.toDouble(&ok);
                const double base = ok ? center : parameter.defaultValue.toDouble(&ok);
                if (ok) {
            ...
            updateHorizontalLine(m_pidDefaultLine, parameter.defaultValue);
            updateHorizontalLine(m_pidCurrentLine, parameter.currentValue);
    ...
    monitorManager.requestDatabaseHistoryPage(this, channelName, {}, {}, 120, {}, 120,
        [this, generation, render](Monitor::DatabaseHistoryPage page) {
            if (generation != m_pidHistoryGeneration) return;
            render(page.samples, page.succeeded() ? QString() :
                   (page.errorText.isEmpty() ? tr("History database query failed") : page.errorText));
        });
}
