...
namespace OutputPaneConfig {
    static constexpr int DEFAULT_MAX_BLOCK_COUNT = 5000;
    static constexpr int MAX_MESSAGE_LENGTH = 65536; // 64 KiB
    static const char* TRUNCATION_MARKER = " ... [TRUNCATED]";

    inline QString sanitizeAndTruncate(const QString& text, int maxLength = MAX_MESSAGE_LENGTH)
    {
        if (text.length() <= maxLength) {
            return text;
        }
        const QString marker = QString::fromLatin1(TRUNCATION_MARKER);
        const int keepLen = qMax(0, maxLength - marker.length());
        return text.left(keepLen) + marker;
    }

    inline bool isScrolledToBottom(const QTextEdit* edit)
    {
        if (!edit) {
            return true;
        }
        const QScrollBar* sb = edit->verticalScrollBar();
        if (!sb) {
            return true;
        }
...
private:
    void appendFormatted(const QString& tag, const QString& message, const QColor& color, bool bold = false);
