...
constexpr int kAsyncCompilerOutputTailLimit =
        kAsyncCompilerOutputLimit - static_cast<int>(sizeof(kAsyncCompilerOutputMarker) - 1);
constexpr qint64 kAsyncCompilerReadChunkSize = 64 * 1024;

void trimToUtf8Boundary(QByteArray* bytes)
{
    if (!bytes || bytes->isEmpty()) {
        return;
    }

    const int inspectLimit = qMin(4, bytes->size());
    int boundary = 0;
    for (; boundary < inspectLimit; ++boundary) {
        const uchar first = static_cast<uchar>(bytes->at(boundary));
        if (first < 0x80) {
            break;
        }
        ...
        if (boundary + sequenceLength > bytes->size()) {
            continue;
        }

        bool valid = true;
        for (int index = 1; index < sequenceLength; ++index) {
            if ((static_cast<uchar>(bytes->at(boundary + index)) & 0xc0) != 0x80) {
                valid = false;
                break;
            }
        }
    ...
    if (boundary > 0 && boundary < bytes->size()) {
        bytes->remove(0, boundary);
    }
}

void appendBoundedOutput(QByteArray* output, bool* truncated, const QByteArray& chunk)
{
    if (!output || !truncated || chunk.isEmpty()) {
        return;
    }

    output->append(chunk);
    if (output->size() <= kAsyncCompilerOutputTailLimit) {
        return;
    }

    *truncated = true;
    *output = output->right(kAsyncCompilerOutputTailLimit);
    trimToUtf8Boundary(output);
}

void drainProcessChannel(QProcess* process,
                         ...
                         bool* truncated)
{
    ...
    while (process->bytesAvailable() > 0) {
        const QByteArray chunk = process->read(kAsyncCompilerReadChunkSize);
        if (chunk.isEmpty()) {
            break;
        }
        appendBoundedOutput(output, truncated, chunk);
    }
}

void drainProcessOutput(QProcess* process,
                        QByteArray* standardOutput,
                        bool* standardOutputTruncated,
                        QByteArray* standardError,
                        bool* standardErrorTruncated)
{
    drainProcessChannel(process,
...
QString boundedOutputText(const QByteArray& output, bool truncated)
{
    QByteArray bytes = output;
    if (truncated) {
        bytes.prepend(kAsyncCompilerOutputMarker);
    }
    return TextEncoding::decodeUtf8WithLocalFallback(bytes);
}
        ...
        std::function<void(const QString&)> onResolved)
{
    ...
    const QString cached = cachedPythonInterpreter(m_pythonProbeWorkDir);
    if (!cached.isEmpty()) {
        onResolved(cached);
        return;
    }
...
void DSLCompilerInterface::onPythonProbeFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == QProcess::NormalExit && exitCode == 0) {
        ...
        qInfo() << "[DSLCompilerInterface] Detected Python interpreter:" << python
                << "for runtime:" << m_pythonProbeWorkDir;
        setCachedPythonInterpreter(m_pythonProbeWorkDir, python);
                                                     ...
                                                     quint64 operationGeneration)
{
    ...
    m_asyncStdOutBytes.clear();
    m_asyncStdErrBytes.clear();
    m_asyncStdOutTruncated = false;
    m_asyncStdErrTruncated = false;
...
void DSLCompilerInterface::cancelCurrentCompile()
{
    ...
    m_process = nullptr;
    m_asyncOperationGeneration = 0;
    process->setProperty("compileCancelled", true);
...
void DSLCompilerInterface::onProcessReadyReadStandardError()
{
    ...
    drainProcessChannel(process,
                        QProcess::StandardError,
                        &m_asyncStdErrBytes,
                        &m_asyncStdErrTruncated);
}

void DSLCompilerInterface::onProcessErrorOccurred(QProcess::ProcessError error)
{
    QProcess* process = qobject_cast<QProcess*>(sender());
    const bool cancelled = process && process->property("compileCancelled").toBool();
    if (!process || (process != m_process && !cancelled))
        return;
    ...
    const QString msg =
            ...
            .arg(process->errorString());
    LOG_WARN(msg);
    appendBoundedOutput(&m_asyncStdErrBytes,
                        &m_asyncStdErrTruncated,
                        (msg + QLatin1Char('\n')).toUtf8());
}
...
void DSLCompilerInterface::onProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (cancelled) {
        QByteArray discardedStdOut;
        QByteArray discardedStdErr;
        bool discardedStdOutTruncated = false;
        bool discardedStdErrTruncated = false;
        drainProcessOutput(process,
                           &discardedStdOut,
                           &discardedStdOutTruncated,
                           &discardedStdErr,
                           &discardedStdErrTruncated);
        process->deleteLater();
        return;
    }
    ...
    drainProcessOutput(process,
                       ...
                       &m_asyncStdErrBytes,
                       &m_asyncStdErrTruncated);
    m_asyncStdOut = boundedOutputText(m_asyncStdOutBytes, m_asyncStdOutTruncated);
    m_asyncStdErr = boundedOutputText(m_asyncStdErrBytes, m_asyncStdErrTruncated);
    ...
    if (process->property("compileTimedOut").toBool()) {
        appendBoundedOutput(&m_asyncStdErrBytes,
                            &m_asyncStdErrTruncated,
                            QByteArrayLiteral("DSL compile process timeout.\n"));
        m_asyncStdErr = boundedOutputText(m_asyncStdErrBytes, m_asyncStdErrTruncated);
    }
    ...
    if (success) {
        ...
        if (!outputExists) {
            appendBoundedOutput(&m_asyncStdErrBytes,
                                &m_asyncStdErrTruncated,
                                QStringLiteral("Expected output file was not generated: %1\n")
                                        .arg(expectedOutput)
                                        .toUtf8());
            m_asyncStdErr = boundedOutputText(m_asyncStdErrBytes, m_asyncStdErrTruncated);
        }
