                                                       ...
                                                       const QString& generationId) const
{
    ...
    if (!profileSource.isEmpty() && !profileSourceRelative.isEmpty()
            && isSafeManifestRelativePath(profileSourceRelative)) {
    ...
    if (profileSource.isEmpty()) {
        ...
        for (CompileArtifact& artifact : result.artifacts) {
            if (artifact.type == QStringLiteral("download"))
                artifact.type = QStringLiteral("compiled_code");
        }
        return result;
    }
    ...
    int parameterCount) const
{
    ...
    for (const auto& s : scriptFiles) {
        const QString relative = DSLCompilerInternal::projectRelativePath(projectPath, s);
        scriptsArr.append(relative);
        const QString fullPath = DSLCompilerInternal::resolveScriptPath(projectPath, s);
        const QString hash = DSLCompilerInternal::sha256ForFile(fullPath);
        if (!relative.isEmpty() && !hash.isEmpty()) {
            sourceChecksums[relative] = hash;
        }
    }
    manifest[QStringLiteral("scriptFiles")] = scriptsArr;
    manifest[QStringLiteral("sourceChecksums")] = sourceChecksums;
