        ...
        if (database.open()) {
            ...
            for (const QString& statement : statements) {
                ...
                if (!query.exec(statement)) {
                    qWarning() << "executeTestSql failed on statement:" << statement
...
bool planUsesIndex(const QString& dbPath, const QString& sql, const QString& indexName)
{
    const QString connectionName = QStringLiteral("DataManagerTest_Plan_%1")
                                       .arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
    bool found = false;
    {
        QSqlDatabase database = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), connectionName);
            ...
            if (query.exec(QStringLiteral("EXPLAIN QUERY PLAN ") + sql)) {
                while (query.next()) {
                    for (int i = 0; i < query.record().count(); ++i) {
                        if (query.value(i).toString().contains(indexName)) {
                            found = true;
                            break;
                        }
                    }
...
void DataManagerTest::testVersion5TimestampMigration()
{
    DataManager::instance().shutdown();

    const QString dbPath = m_tempDir.path() + "/v5_migration_test.db";
    QFile::remove(dbPath);

    // Create a V4 database with legacy space timestamps
    QVERIFY(executeTestSql(dbPath, {
        QStringLiteral("CREATE TABLE schema_version (id INTEGER PRIMARY KEY, version INTEGER NOT NULL, updated_at DATETIME)"),
        QStringLiteral("INSERT INTO schema_version (id, version, updated_at) VALUES (1, 4, '2026-09-08 10:00:00')"),
        QStringLiteral("CREATE TABLE runtime_data ("
                       ...
                       "value_valid INTEGER DEFAULT 1, "
                       "origin TEXT DEFAULT 'polled', "
                       "error_code INTEGER DEFAULT 0, "
                       "error_text TEXT DEFAULT '')"),
        QStringLiteral("CREATE TABLE system_logs ("
                       "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                       "timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, "
                       "level TEXT NOT NULL, "
                       "module TEXT, "
                       "message TEXT)"),
        QStringLiteral("CREATE TABLE data_summary ("
                       "id INTEGER PRIMARY KEY AUTOINCREMENT, "
                       "date TEXT NOT NULL, "
                       "variable_name TEXT NOT NULL, "
                       "min_value REAL, "
        ...
        QStringLiteral("CREATE INDEX idx_runtime_timestamp ON runtime_data(timestamp)"),
        QStringLiteral("CREATE INDEX idx_runtime_variable ON runtime_data(variable_name)"),
        QStringLiteral("CREATE INDEX idx_runtime_var_time ON runtime_data(variable_name, timestamp)"),
        QStringLiteral("CREATE INDEX idx_logs_timestamp ON system_logs(timestamp)"),
        QStringLiteral("CREATE INDEX idx_logs_level ON system_logs(level)"),
        QStringLiteral("CREATE INDEX idx_logs_level_time ON system_logs(level, timestamp)"),
        QStringLiteral("INSERT INTO runtime_data (timestamp, variable_name, value, unit) VALUES ('2020-01-02 03:04:05', 'var1', 42.0, 'bar')"),
        QStringLiteral("INSERT INTO runtime_data (timestamp, variable_name, value, unit) VALUES ('2020-01-02 03:04:05.123', 'var1', 43.0, 'bar')"),
        QStringLiteral("INSERT INTO system_logs (timestamp, level, module, message) VALUES ('2020-01-02 03:04:05', 'INFO', 'mod', 'msg1')")
    }));

    // Initialize - should trigger upgradeToVersion5()
    QVERIFY(DataManager::instance().initialize(dbPath));
    QVERIFY(DataManager::instance().isInitialized());
    QCOMPARE(DataManager::instance().schemaVersion(), 5);

    // Verify timestamps were normalized to canonical UTC ISO with 'Z'
    QVariant t1, t2, logTime;
    QVERIFY(queryTestScalar(dbPath, QStringLiteral("SELECT timestamp FROM runtime_data WHERE id = 1"), t1));
    QVERIFY(queryTestScalar(dbPath, QStringLiteral("SELECT timestamp FROM runtime_data WHERE id = 2"), t2));
    QVERIFY(queryTestScalar(dbPath, QStringLiteral("SELECT timestamp FROM system_logs WHERE id = 1"), logTime));

    QCOMPARE(t1.toString(), QStringLiteral("2020-01-02T03:04:05.000Z"));
    QCOMPARE(t2.toString(), QStringLiteral("2020-01-02T03:04:05.123Z"));
    QCOMPARE(logTime.toString(), QStringLiteral("2020-01-02T03:04:05.000Z"));

    // Verify queryHistory works on migrated records
    const QDateTime start = QDateTime::fromString(QStringLiteral("2020-01-02T03:04:04.000Z"), Qt::ISODateWithMs);
    const QDateTime end = QDateTime::fromString(QStringLiteral("2020-01-02T03:04:06.000Z"), Qt::ISODateWithMs);
    auto records = DataManager::instance().queryHistory("var1", start, end);
    QCOMPARE(records.size(), 2);

    // Verify queryLogs works on migrated logs
    auto logs = DataManager::instance().queryLogs(start, end);
    QCOMPARE(logs.size(), 1);
    QCOMPARE(logs.first().message, QStringLiteral("msg1"));

    // Verify range query uses idx_runtime_var_time
    const QString historySql = QStringLiteral(
        "SELECT id, timestamp, variable_name, value, unit, quality, value_valid, origin, error_code, error_text "
        "FROM runtime_data "
        "WHERE variable_name = 'var1' AND timestamp BETWEEN '2020-01-02T03:04:04.000Z' AND '2020-01-02T03:04:06.000Z' "
        "ORDER BY timestamp ASC, id ASC");
    QVERIFY(planUsesIndex(dbPath, historySql, QStringLiteral("idx_runtime_var_time")));

    DataManager::instance().shutdown();

    // Re-initialize to verify idempotency (no repeat migration failure)
    QVERIFY(DataManager::instance().initialize(dbPath));
    QVERIFY(DataManager::instance().isInitialized());
    QCOMPARE(DataManager::instance().schemaVersion(), 5);
    DataManager::instance().shutdown();
...
void DataManagerTest::testVersion5TimestampMigrationRollback()
{
    DataManager::instance().shutdown();

    const QString dbPath = m_tempDir.path() + "/v5_migration_rollback_test.db";
    QFile::remove(dbPath);

    // Create a V4 database with unparseable corrupt timestamp
    QVERIFY(executeTestSql(dbPath, {
        QStringLiteral("CREATE TABLE schema_version (id INTEGER PRIMARY KEY, version INTEGER NOT NULL, updated_at DATETIME)"),
        QStringLiteral("INSERT INTO schema_version (id, version, updated_at) VALUES (1, 4, '2026-09-08 10:00:00')"),
        QStringLiteral("CREATE TABLE runtime_data ("
        ...
        QStringLiteral("CREATE TABLE data_summary ("
                       ...
                       "count INTEGER, "
                       "UNIQUE(date, variable_name))"),
        QStringLiteral("INSERT INTO runtime_data (timestamp, variable_name, value, unit) VALUES ('not-a-valid-timestamp', 'var1', 42.0, 'bar')")
    }));

    // Initialize should fail because of invalid timestamp
    QVERIFY(!DataManager::instance().initialize(dbPath));
    QVERIFY(!DataManager::instance().isInitialized());

    // Verify rollback: version remains 4
    QVariant ver;
    QVERIFY(queryTestScalar(dbPath, QStringLiteral("SELECT version FROM schema_version WHERE id = 1"), ver));
    QCOMPARE(ver.toInt(), 4);

    // Verify corrupt timestamp was not modified
    QVariant t;
    QVERIFY(queryTestScalar(dbPath, QStringLiteral("SELECT timestamp FROM runtime_data WHERE id = 1"), t));
    QCOMPARE(t.toString(), QStringLiteral("not-a-valid-timestamp"));
...
void DataManagerTest::testRuntimeDataInputValidation()
{
    DataManager& manager = DataManager::instance();
    QVERIFY(manager.logRuntimeData(QStringLiteral("protected"), 42.0).success);

    const auto assertSingleRejected = [&](const QueryResult& result) {
        QVERIFY(!result.success);
        QCOMPARE(result.errorCode, QStringLiteral("InvalidRuntimeData"));
        QVERIFY(!result.errorText.isEmpty());
    };
    assertSingleRejected(manager.logRuntimeData(QString(), 1.0));
    assertSingleRejected(manager.logRuntimeData(QStringLiteral(" \t\n"), 1.0));
    assertSingleRejected(manager.logRuntimeData(QStringLiteral("single.nan"),
                                                std::numeric_limits<double>::quiet_NaN()));
    assertSingleRejected(manager.logRuntimeData(QStringLiteral("single.inf"),
                                                std::numeric_limits<double>::infinity()));

    const auto assertBatchRejected = [&](const QVariantMap& record,
                                         const QString& expectedText) {
        const QueryResult result = manager.logRuntimeDataBatch({record});
        QVERIFY(!result.success);
        QCOMPARE(result.errorCode, QStringLiteral("InvalidRuntimeData"));
        QVERIFY(result.errorText.contains(expectedText));
        ...
        const QList<RuntimeRecord> protectedRecords = manager.getLatestRecords(
            QStringLiteral("protected"), 1);
        QCOMPARE(protectedRecords.size(), 1);
        QCOMPARE(protectedRecords.first().value, 42.0);
    };

    QVariantMap emptyName;
    emptyName.insert(QStringLiteral("varName"), QString());
    emptyName.insert(QStringLiteral("value"), 1.0);
    assertBatchRejected(emptyName, QStringLiteral("varName"));

    QVariantMap whitespaceName;
    whitespaceName.insert(QStringLiteral("varName"), QStringLiteral(" \t\n"));
    whitespaceName.insert(QStringLiteral("value"), 1.0);
    assertBatchRejected(whitespaceName, QStringLiteral("不能为空"));

    QVariantMap missingValue;
    missingValue.insert(QStringLiteral("varName"), QStringLiteral("missing"));
    assertBatchRejected(missingValue, QStringLiteral("不能为空"));

    QVariantMap unconvertibleValue;
    unconvertibleValue.insert(QStringLiteral("varName"), QStringLiteral("unconvertible"));
    unconvertibleValue.insert(QStringLiteral("value"), QStringLiteral("not-a-number"));
    assertBatchRejected(unconvertibleValue, QStringLiteral("可转换"));

    QVariantMap nanValue;
    nanValue.insert(QStringLiteral("varName"), QStringLiteral("nan"));
    nanValue.insert(QStringLiteral("value"), std::numeric_limits<double>::quiet_NaN());
    assertBatchRejected(nanValue, QStringLiteral("有限数值"));

    QVariantMap infValue;
    infValue.insert(QStringLiteral("varName"), QStringLiteral("inf"));
    infValue.insert(QStringLiteral("value"), std::numeric_limits<double>::infinity());
    assertBatchRejected(infValue, QStringLiteral("有限数值"));

    QVariantMap oversizedText;
    oversizedText.insert(QStringLiteral("varName"), QStringLiteral("oversized"));
    oversizedText.insert(QStringLiteral("value"), 1.0);
    oversizedText.insert(QStringLiteral("unit"), QString(1025, QLatin1Char('x')));
    assertBatchRejected(oversizedText, QStringLiteral("unit"));

    QVariantMap invalidSample;
    invalidSample.insert(QStringLiteral("varName"), QStringLiteral("invalid.sample"));
    invalidSample.insert(QStringLiteral("valueValid"), false);
    invalidSample.insert(QStringLiteral("value"), QStringLiteral("not-a-number"));
    QVERIFY(manager.logRuntimeDataBatch({invalidSample}).success);
    const QList<RuntimeRecord> invalidRecords = manager.getLatestRecords(
        QStringLiteral("invalid.sample"), 1);
    QCOMPARE(invalidRecords.size(), 1);
    QVERIFY(!invalidRecords.first().valueValid);
    QVariant invalidValueIsNull;
    QVERIFY(queryTestScalar(m_dbPath,
                            QStringLiteral("SELECT value IS NULL FROM runtime_data "
                                           "WHERE variable_name = 'invalid.sample'"),
                            invalidValueIsNull));
    QCOMPARE(invalidValueIsNull.toInt(), 1);

    QVariantMap nonFiniteSample;
    nonFiniteSample.insert(QStringLiteral("varName"), QStringLiteral("nonfinite.sample"));
    nonFiniteSample.insert(QStringLiteral("valueValid"), false);
    nonFiniteSample.insert(QStringLiteral("value"), std::numeric_limits<double>::infinity());
    QVERIFY(manager.logRuntimeDataBatch({nonFiniteSample}).success);
    QVariant nonFiniteValueIsNull;
    QVERIFY(queryTestScalar(m_dbPath,
                            QStringLiteral("SELECT value IS NULL FROM runtime_data "
                                           "WHERE variable_name = 'nonfinite.sample'"),
                            nonFiniteValueIsNull));
    QCOMPARE(nonFiniteValueIsNull.toInt(), 1);

    QVariantMap zero;
    zero.insert(QStringLiteral("varName"), QStringLiteral("zero"));
    zero.insert(QStringLiteral("value"), 0);
    QVariantMap negative;
    negative.insert(QStringLiteral("varName"), QStringLiteral("negative"));
    negative.insert(QStringLiteral("value"), -2.5);
    QVERIFY(manager.logRuntimeDataBatch({zero, negative}).success);
    QCOMPARE(manager.getRuntimeValue(QStringLiteral("zero")).toDouble(), 0.0);
    QCOMPARE(manager.getRuntimeValue(QStringLiteral("negative")).toDouble(), -2.5);

    QVariant countBeforeMixedBatch;
    QVERIFY(queryTestScalar(m_dbPath,
                            QStringLiteral("SELECT COUNT(*) FROM runtime_data"),
                            countBeforeMixedBatch));
    QSignalSpy mixedBatchSpy(&manager, &DataManager::dataUpdated);
    QVariantMap validMixedRecord;
    validMixedRecord.insert(QStringLiteral("varName"), QStringLiteral("atomic.valid"));
    validMixedRecord.insert(QStringLiteral("value"), 7.5);
    QVariantMap invalidMixedRecord;
    invalidMixedRecord.insert(QStringLiteral("varName"), QStringLiteral(" \t"));
    invalidMixedRecord.insert(QStringLiteral("value"), 8.5);

    const QueryResult mixedResult = manager.logRuntimeDataBatch(
        {validMixedRecord, invalidMixedRecord});
    QVERIFY(!mixedResult.success);
    QCOMPARE(mixedResult.errorCode, QStringLiteral("InvalidRuntimeData"));
    QCOMPARE(mixedBatchSpy.count(), 0);

    QVariant countAfterMixedBatch;
    QVERIFY(queryTestScalar(m_dbPath,
                            QStringLiteral("SELECT COUNT(*) FROM runtime_data"),
                            countAfterMixedBatch));
    QCOMPARE(countAfterMixedBatch, countBeforeMixedBatch);
    QVERIFY(!manager.getRuntimeValue(QStringLiteral("atomic.valid")).isValid());
    QCOMPARE(manager.getRuntimeValue(QStringLiteral("protected")).toDouble(), 42.0);
}
...
void DataManagerTest::testCleanupOldDataIsTransactionalAndUsesUtc()
{
    ...
    const QString oldTimestamp = nowUtc.addDays(-1).addSecs(-3600).toString(Qt::ISODateWithMs);
    const QString retainedTimestamp = nowUtc.addDays(-1).addSecs(3600).toString(Qt::ISODateWithMs);
...
void DataManagerTest::testCrossThreadAccessRejected()
{
    std::atomic<bool> crossThreadLogResult{true};
    std::atomic<bool> crossThreadInitResult{true};
    std::atomic<bool> crossThreadQueryEmpty{false};
    std::atomic<bool> crossThreadCleanupNegative{false};

    QThread* worker = QThread::create([&]() {
        // 1. 尝试跨线程写运行时数据
        const QueryResult logRes = DataManager::instance().logRuntimeData(QStringLiteral("thread_var"), 123.0);
        crossThreadLogResult.store(logRes.success);

        // 2. 尝试跨线程初始化
        crossThreadInitResult.store(DataManager::instance().initialize(m_dbPath));

        // 3. 尝试跨线程查询历史
        const auto results = DataManager::instance().queryHistory(QStringLiteral("thread_var"),
                                                                  QDateTime::currentDateTimeUtc().addDays(-1),
                                                                  QDateTime::currentDateTimeUtc());
        crossThreadQueryEmpty.store(results.isEmpty());

        // 4. 尝试跨线程清理旧数据
        crossThreadCleanupNegative.store(DataManager::instance().cleanupOldData(1) == -1);
    });
    ...
    // 验证所有跨线程调用均被 checkThreadOwnership() 严格拒绝
    QVERIFY(!crossThreadLogResult.load());
    QVERIFY(!crossThreadInitResult.load());
    QVERIFY(crossThreadQueryEmpty.load());
    QVERIFY(crossThreadCleanupNegative.load());

    // 验证在属主线程（当前主线程）中正常操作依然成功
    QVERIFY(DataManager::instance().logRuntimeData(QStringLiteral("owner_var"), 456.0).success);
}
