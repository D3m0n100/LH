"""
LH 字节码解释器 v2  —— 文本格式 FB 实例化列表
===============================================
解析并模拟执行 LH 编译器实际输出的 .code 文件。
...
def main():
    parser = argparse.ArgumentParser(
        description='LH .code 文件解释器与验证工具（文本 FB 序列格式）'
    )
    ...
    print(f'\n{"="*68}')
    print(f'  LH 字节码解释器  —  共发现 {len(files)} 个 .code 文件')
    print(f'{"="*68}\n')
