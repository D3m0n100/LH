...
lh_programs/
├── main.lh              主程序（整合所有模块）
├── system_init.lh       系统初始化模块
├── io_module.lh         I/O驱动模块
├── data_process.lh      数据处理模块
├── control_module.lh    控制逻辑模块
├── safety_module.lh     安全监控模块
├── comm_module.lh       通信模块
├── display_module.lh    显示交互模块
├── exca_module.lh       EXCA专用模块
└── README.md            本文档
```
...
```bash
# 编译单个模块
python lmc.py lh_programs/system_init.lh -v

# 编译所有模块
python lmc.py lh_programs/*.lh -o compiled/
```
...
```bash
# Windows
for %f in (lh_programs\*.lh) do python lmc.py %f -o compiled\

# Linux/Mac
for f in lh_programs/*.lh; do python lmc.py "$f" -o compiled/; done
```
...
### 修改I/O配置
编辑 `io_module.lh`，根据实际硬件修改：
- AI通道数量和量程
- DI通道数量和逻辑
- DO输出配置

### 添加控制算法
在 `control_module.lh` 中添加：
- PID控制器
- 模糊控制
- 自定义算法

### 扩展通信协议
在 `comm_module.lh` 中添加：
- Modbus RTU/TCP
- Profibus
- EtherCAT

### 定制显示界面
在 `display_module.lh` 中修改：
- 显示内容和布局
- 菜单结构
- 用户交互逻辑
...
### 推荐阅读顺序
1. 先看 `system_init.lh` - 了解基本结构
2. 再看 `io_module.lh` - 学习I/O处理
3. 然后看 `data_process.lh` - 掌握数据处理
4. 最后看 `main.lh` - 理解整体架构
