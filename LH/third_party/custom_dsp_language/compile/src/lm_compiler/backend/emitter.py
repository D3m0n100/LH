...
# 同级导入
try:
    from lm_compiler.backend.codegen import Instruction
except ImportError:
...
class CodeEmitter:
    ...
    def emit_with_header(
        ...
        header_file = ""
        header_datetime = ""
        header_item = "构件码构件数据项"
        if metadata:
            header_version = str(metadata.get("版本号", ""))
            header_file = str(metadata.get("文件名", ""))
            header_datetime = str(metadata.get("日期_时间", ""))
            header_item = str(metadata.get("说明", header_item))

        if header_version:
            lines.append(f"//版本号: {header_version}")
        if header_file:
            lines.append(f"//文件名: {header_file}")
        if header_datetime:
            lines.append(f"//日期_时间: {header_datetime}")
        lines.append(f"//{header_item}")

        if self.add_comments and program_name:
