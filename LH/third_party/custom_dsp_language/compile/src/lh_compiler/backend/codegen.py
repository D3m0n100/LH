...
用法:
    from lh_compiler.backend import CodeGenerator
    from lh_compiler.function_blocks import FunctionBlockRegistry
...
try:
    from lh_compiler.frontend.ast_nodes import ArrayIndex, MemberAccess, FuncCallExpr
except ImportError:
    try:
        from src.lh_compiler.frontend.ast_nodes import ArrayIndex, MemberAccess, FuncCallExpr
    except ImportError:
        class ArrayIndex:
            pass
        class MemberAccess:
...
class CodeGenerator:
    ...
    def _emit_system(self, program: Program):
        ...
        if not system_meta:
            self.errors.append("未找?System 功能块定")
            return
