...
将此文件放在 E:\\lm-compiler\\ 目录下（与 lmc.py 同级）
"""
...
def run_compile(source_path, output_path=None, verbose=True, debug=False, grammar_path=None):
    """
    调用 LMCompiler 执行编译，返回 (success, log_text, output_file)
    """
    try:
...
class LmIdeApp:
    ...
    def _build_menu(self):
        ...
        file_menu.add_command(label="打开 LM 文件...", command=self._open_file, accelerator="Ctrl+O")
    ...
    def _build_main_area(self):
        ...
        self.project_dir_var = tk.StringVar(value=str(PROJECT_ROOT / "lm_programs") if (PROJECT_ROOT / "lm_programs").is_dir() else str(PROJECT_ROOT))
        ...
        # ── 中栏：代码编辑器 ──
        center_frame = ttk.LabelFrame(main, text="LM 源代码", padding=4)
    ...
    def _compile(self, verbose=None, debug=None):
        ...
        if not source or not os.path.isfile(source):
            messagebox.showwarning("提示", "请先打开一个 .lm 源文件")
            return
    ...
    def _show_about(self):
        messagebox.showinfo("关于 LM-IDE",
            "LM-IDE  嵌入式可组态软件平台 v1.0\n\n"
            "面向电液伺服阀控制系统的\n"
            "领域特定语言编译环境\n\n"
            "作者: 朱斌\n"
            "上海应用技术大学 轨道交通学院\n\n"
