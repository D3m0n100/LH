...
static bool prependCodeMetadataHeader(const QString& outputFile,
                                          ...
                                          QString* compilerStderr)
{
    ...
    if (ok) {
        QString headerError;
        ok = prependCodeMetadataHeader(outputFile, sourceFile, projectName, compilerInputFile, &headerError);
        if (!ok) {
                                                             ...
                                                             const QString& projectName)
{
    ...
    if (ok) {
        QString headerError;
        ok = prependCodeMetadataHeader(outputFile, mainScriptFile, projectName, compilerInputFile, &headerError);
        if (!ok) {
            stdErr.append(headerError + QLatin1Char('\n'));
        }
    }
...
void DSLCompilerInterface::onProcessFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (success) {
        QString headerError;
        success = prependCodeMetadataHeader(expectedOutput,
